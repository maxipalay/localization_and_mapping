...
private:
  LoggerConfig loadConfig_()
  {
    ...
    config.rgb_match_tolerance_ns =
      msToNs(declare_parameter<double>("rgb_match_tolerance_ms", 3.0));
  ...
  void handleAssemblerUpdate_(const AssemblerUpdate &update)
  {
    for (const auto &record : update.completed_records) {
      ...
      RCLCPP_INFO(
        ...
        record_with_tag_poses.pending.have_tags && record_with_tag_poses.pending.tags_msg
          ? record_with_tag_poses.pending.tags_msg->detections.size()
          : 0,
        resolved_tag_pose_count);
    }
  ...
  void resolveTagPoses_(PendingKeyframe &pending)
  {
    ...
    if (!pending.have_tags || !pending.tags_msg || !tf_buffer_) {
      return;
    }
