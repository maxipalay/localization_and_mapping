...
void TracksBuffer::setRightAll(const std::vector<cv::Point2f>& pr_new,
                               const std::vector<uint8_t>* valid) {
  const size_t n = ids_.size();
  assert(pr_new.size() == n);
  assert(pr_.size() == n && has_r_.size() == n);
  ...
  if (valid) {
    assert(valid->size() == n);
    for (size_t i = 0; i < n; ++i) {
      has_r_[i] = ((*valid)[i] != 0) ? uint8_t{1} : uint8_t{0};
    }
  } else {
...
void TracksBuffer::setRightForIndices(const std::vector<int>& indices,
                                      const std::vector<cv::Point2f>& pr_values) {
  assert(indices.size() == pr_values.size());
  const int n = static_cast<int>(ids_.size());

  for (size_t k = 0; k < indices.size(); ++k) {
    const int idx = indices[k];
    assert(idx >= 0 && idx < n);
    pr_[static_cast<size_t>(idx)] = pr_values[k];
    has_r_[static_cast<size_t>(idx)] = uint8_t{1};
  }
}
...
void TracksBuffer::gatherPnP(std::vector<cv::Point3f>& object_pts,
  ...
  image_pts.reserve(n);
  if (out_indices) out_indices->reserve(n);

  for (size_t i = 0; i < n; ++i) {
    if (!has_X_prev_[i]) continue;
    object_pts.push_back(X_prev_[i]);
    image_pts.push_back(pl_[i]);
    if (out_indices) out_indices->push_back(static_cast<int>(i));
  }
}
