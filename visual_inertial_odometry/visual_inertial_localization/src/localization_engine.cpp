...
LocalizationEngine::LocalizationEngine(LocalizationConfig config)
    : config_(std::move(config))
{
}

LocalizationLoadReport LocalizationEngine::loadTagMap()
{
    mapped_tags_.clear();
...
TagIngestReport LocalizationEngine::ingestDetections(
...
std::vector<BufferedTagObservation> LocalizationEngine::recentObservationsForStamp(
...
std::optional<BootstrapEstimate> LocalizationEngine::estimateBootstrap(
...
std::optional<PosePriorEstimate> LocalizationEngine::estimatePosePrior(const rclcpp::Time &stamp) const
{
...
std::vector<PosePriorEstimate> LocalizationEngine::estimatePosePriors(const rclcpp::Time &stamp) const
{
    const auto observations = recentObservationsForStamp(stamp);
...
std::optional<StableCorrectionEstimate> LocalizationEngine::estimateStableCorrection(
    const rclcpp::Time &stamp) const
{
...
std::optional<StableCorrectionEstimate> LocalizationEngine::estimateRelocalizationCorrection(
    const rclcpp::Time &stamp) const
{
    std::lock_guard<std::mutex> lk(stable_correction_mtx_);
    const int64_t stamp_ns = stamp.nanoseconds();
...
size_t LocalizationEngine::bufferedObservationCount() const noexcept
{
    std::lock_guard<std::mutex> lk(tag_obs_mtx_);
...
const std::unordered_map<int, Eigen::Isometry3d> &LocalizationEngine::mappedTags() const noexcept
{
    return mapped_tags_;
}

std::string LocalizationEngine::tagFrameName_(const apriltag_msgs::msg::AprilTagDetection &detection) const
{
...
bool LocalizationEngine::isObliqueTagObservation_(const Eigen::Isometry3d &T_BT) const
{
...
void LocalizationEngine::pruneBufferedTagObservationsLocked_(int64_t newest_stamp_ns) const
{
...
std::vector<LocalizationEngine::PoseHypothesis> LocalizationEngine::buildPoseHypotheses_(
...
std::vector<size_t> LocalizationEngine::dominantHypothesisCluster_(
...
size_t LocalizationEngine::bestHypothesisIndex_(
...
void LocalizationEngine::pruneTemporalCorrectionHypothesesLocked_(int64_t newest_stamp_ns) const
{
...
std::optional<StableCorrectionEstimate> LocalizationEngine::computeCorrectionEstimateLocked_(
...
std::optional<StableCorrectionEstimate> LocalizationEngine::updateStableCorrection_(
    int64_t stamp_ns,
